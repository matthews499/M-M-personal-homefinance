...
function NewPotModal({ onSave, onClose }) {
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <p className="text-xs" style={{ color: t.textMuted }}>A fixed outgoing entry will be created with the calculated monthly contribution as a guide.</p>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button onClick={handleSave} disabled={saving} className="flex-1 py-3 rounded-xl text-sm font-semibold disabled:opacity-50" style={{ backgroundColor: t.purple, color: '#fff' }}>{saving ? 'Creating…' : 'Create pot'}</button>
      </div>
    </Sheet>
  )
}
...
// ── Transaction modal (deposit or withdrawal) ─────────────────
...
function TransactionModal({ pot, onSave, onClose }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      await onSave(pot.id, { type: txType, amount: parseFloat(amount), transaction_date: date, note })
      onClose()
    } catch (e) { setErr(e.message); setSaving(false) }
  }
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <div>
        ...
        <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Monthly transfer" className="w-full px-3 py-3 rounded-xl text-base outline-none focus:ring-1 focus:ring-violet-500" style={inputStyle} />
      </div>

      {err && <p className="text-xs px-3 py-2 rounded-lg" style={{ backgroundColor: t.redDim, color: t.red }}>{err}</p>}
      <div className="flex gap-2 pt-1">
        <button onClick={onClose} className="flex-1 py-3 rounded-xl text-sm font-medium" style={{ backgroundColor: 'var(--color-pill-bg)', color: t.textSecondary }}>Cancel</button>
        <button onClick={handleSave} disabled={saving} className="flex-1 py-3 rounded-xl text-sm font-semibold disabled:opacity-50" style={{ backgroundColor: isWithdrawal ? t.red : t.purple, color: '#fff' }}>
          {saving ? 'Saving…' : isWithdrawal ? 'Log withdrawal' : 'Log deposit'}
        </button>
      </div>
    </Sheet>
  )
...
export default function JointSavings() {
  const { pots, transactionsForPot, loading, createPot, updatePot, removePot, addTransaction, removeTransaction } = useJointSavings()
    ...
    <>
      <div className="rounded-2xl p-5 space-y-5" style={cardStyle}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setAdding(true)} className="text-xs px-3 py-1.5 rounded-lg font-semibold" style={{ backgroundColor: t.purpleBg, color: t.purpleText }}>+ New pot</button>
        </div>

        {pots.length === 0 && <p className="text-sm py-1" style={{ color: t.textMuted }}>No savings pots yet.</p>}
                ...
                {isOpen && (
                  <div style={{ borderTop: `1px solid ${t.divider}` }}>
                    {potTxns.length === 0 ? (
                      ...
                      potTxns.map(tx => (
                        <div key={tx.id} className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: `1px solid ${t.divider}` }}>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold" style={{ color: tx.type === 'withdrawal' ? t.red : t.green }}>
                              {tx.type === 'withdrawal' ? '−' : '+'}{currency(tx.amount)}
                            </p>
                            <p className="text-xs mt-0.5" style={{ color: t.textMuted }}>{fmtDate(tx.transaction_date)}{tx.note ? ` · ${tx.note}` : ''}</p>
                          </div>
                          <button onClick={() => handleRemoveTx(tx.id)} className="p-2 rounded-lg active:bg-white/10" style={{ color: t.textMuted }}>
