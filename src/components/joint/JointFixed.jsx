...
// Modal-based add/edit for fixed outgoings
function FixedModal({ item, onSave, onCancel }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      await onSave({ name: name.trim(), amount: parseFloat(amount), day_of_month: parseInt(day, 10) })
    } catch (e) {
      setErr(e.message)
      setSaving(false)
    }
...
export default function JointFixed() {
  const { items, loading, create, update, remove } = useJointFixed()
  const [adding,    setAdding]    = useState(false)
  const [editingItem, setEditingItem] = useState(null)

  const manualItems  = items.filter(i => !i.is_savings_contribution)
  const savingsItems = items.filter(i => i.is_savings_contribution)
    ...
    <>
      <div className="rounded-2xl p-5 space-y-5" style={cardStyle}>
          ...
          {manualItems.map(item => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5" style={{ color: t.textMuted }}>{ordinal(item.day_of_month)} of the month</p>
              </div>
        ...
        <div className="flex justify-between items-center pt-1">
          ...
          <span className="text-xl font-bold tabular-nums" style={{ color: t.textPrimary }}>{currency(sumAmount(items))}</span>
        </div>
      </div>
