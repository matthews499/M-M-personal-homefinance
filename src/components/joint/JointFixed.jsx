...
export default function JointFixed({ period }) {
  ...
  // Upcoming: active_from set and is in the future
  const upcomingItems = notExpired.filter(i => i.active_from && i.active_from > currentPeriod && !i.is_savings_contribution)
  // Active: no active_from, OR active_from <= currentPeriod
  const activeItems   = notExpired.filter(i => (!i.active_from || i.active_from <= currentPeriod) && !i.is_savings_contribution)
  const savingsItems  = notExpired.filter(i => i.is_savings_contribution && (!i.active_from || i.active_from <= currentPeriod))
  ...
  async function handleDelete(id) { if (confirm('Delete this fixed outgoing?')) await remove(id) }
  ...
  const activeTotal = sumAmount([...activeItems, ...savingsItems])
  ...
  return (
    <>
      <div className="rounded-2xl p-5 space-y-5" style={cardStyle}>
        ...
        {activeItems.length === 0 && savingsItems.length === 0 && (
        ...
        <div className="flex justify-between items-center pt-1">
          ...
          <span className="text-xl font-bold tabular-nums" style={{ color: t.textPrimary }}>{currency(activeTotal)}</span>
        </div>
            ...
            <div className="space-y-0">
              {upcomingItems.map(item => (
                <div key={item.id} className="flex items-center justify-between py-3" style={{ borderBottom: `1px solid ${t.divider}` }}>
                  ...
                  <span className="text-sm font-bold tabular-nums mr-3" style={{ color: t.textSecondary }}>{currency(item.amount)}</span>
                  <div className="flex gap-1">
